"""
Data loader for multi-file deltabot inverse model training.

This module loads data from multiple files (different voltages) and creates
TensorFlow datasets for efficient training with proper scaling.
"""

import numpy as np
import pandas as pd
import tensorflow as tf
from pathlib import Path
from typing import List, Tuple, Optional
from numpy.lib.stride_tricks import sliding_window_view
from sklearn.preprocessing import StandardScaler


class MultiFIleDatabotLoader:
    """Load data from multiple deltabot files and create tf.data.Dataset objects."""

    def __init__(self, file_paths: List[str], window_length: int = 3):
        """
        Initialize the multi-file data loader.

        Args:
            file_paths: List of paths to data files
            window_length: Length of sliding window for temporal features
        """
        self.file_paths = file_paths
        self.window_length = window_length

        # Scalers for features and targets
        self.X_scaler = StandardScaler()
        self.y_scaler = StandardScaler()

    def process_single_file(self, file_path: str) -> Tuple[np.ndarray, np.ndarray]:
        """
        Process a single data file and extract features/targets.

        Args:
            file_path: Path to the data file

        Returns:
            Tuple of (positions, voltage_windows_and_derivatives)
        """
        df = pd.read_csv(file_path, sep=r"\s+", header=None)
        df.columns = [
            "timestep",
            "x_in_reg", "x_in",
            "y_in_reg", "y_in",
            "z_in_reg", "z_in",
            "x_out", "y_out", "z_out"
        ]

        # Extract input voltages
        x_in = df["x_in"].values.reshape(-1, 1)
        y_in = df["y_in"].values.reshape(-1, 1)
        z_in = df["z_in"].values.reshape(-1, 1)

        # Extract output positions
        x_out = df["x_out"].values.reshape(-1, 1)
        y_out = df["y_out"].values.reshape(-1, 1)
        z_out = df["z_out"].values.reshape(-1, 1)

        x_in.setflags(write=False)
        y_in.setflags(write=False)
        z_in.setflags(write=False)

        x_in_window = np.squeeze(sliding_window_view(x_in, self.window_length, 0))
        y_in_window = np.squeeze(sliding_window_view(y_in, self.window_length, 0))
        z_in_window = np.squeeze(sliding_window_view(z_in, self.window_length, 0))

        x_out_truncated = np.delete(x_out, slice(0, self.window_length - 1))
        y_out_truncated = np.delete(y_out, slice(0, self.window_length - 1))
        z_out_truncated = np.delete(z_out, slice(0, self.window_length - 1))

        # Concatenate all voltage windows (targets)
        all_data_in = np.concatenate([x_in_window, y_in_window, z_in_window], axis=1)

        # Calculate velocities (first derivative)
        x_velocities = x_in_window[:, 1:] - x_in_window[:, :-1]
        y_velocities = y_in_window[:, 1:] - y_in_window[:, :-1]
        z_velocities = z_in_window[:, 1:] - z_in_window[:, :-1]

        # Calculate accelerations (second derivative)
        x_accelerations = x_velocities[:, 1:] - x_velocities[:, :-1]
        y_accelerations = y_velocities[:, 1:] - y_velocities[:, :-1]
        z_accelerations = z_velocities[:, 1:] - z_velocities[:, :-1]

        # Concatenate: windows + velocities + accelerations
        all_data_in = np.concatenate([
            all_data_in,
            x_velocities, y_velocities, z_velocities,
            x_accelerations, y_accelerations, z_accelerations
        ], axis=1)

        all_data_out = np.stack([
            x_out_truncated,
            y_out_truncated,
            z_out_truncated
        ], axis=1).squeeze()

        return all_data_out, all_data_in

    def load_all_files(self, scale_data: bool = True) -> Tuple[np.ndarray, np.ndarray]:
        """
        Load and concatenate all data files.

        Args:
            scale_data: Whether to scale features and targets (default: True)

        Returns:
            Tuple of (X, y) where:
                X: Position features [x_out, y_out, z_out]
                y: Voltage targets [windows + velocities + accelerations]
        """
        all_x = []
        all_y = []

        for file_path in self.file_paths:
            print(f"Loading {file_path}...")
            x, y = self.process_single_file(file_path)
            all_x.append(x)
            all_y.append(y)
            print(f"  Loaded: X={x.shape}, y={y.shape}")

        X = np.vstack(all_x)
        Y = np.vstack(all_y)

        print(f"\nTotal dataset (before scaling): X={X.shape}, y={Y.shape}")

        if scale_data:
            print("Scaling data...")
            X = self.X_scaler.fit_transform(X)
            Y = self.y_scaler.fit_transform(Y)

        return X, Y

    def create_temporal_split(
        self,
        X: np.ndarray,
        y: np.ndarray,
        train_split: float = 0.8
    ) -> Tuple[np.ndarray, np.ndarray, np.ndarray, np.ndarray]:
        """
        Create temporal train/test split (no shuffling to preserve time order).
        Use 80% for training and 20% for testing.

        Args:
            X: Features
            y: Targets
            train_split: Fraction of data for training (default: 0.8) 80%

        Returns:
            Tuple of (X_train, X_test, y_train, y_test)
        """
        split_idx = int(len(X) * train_split)

        X_train = X[:split_idx]
        X_test = X[split_idx:]
        y_train = y[:split_idx]
        y_test = y[split_idx:]

        print(f"\nTemporal split:")
        print(f"  Train: {X_train.shape[0]} samples ({train_split*100:.0f}%)")
        print(f"  Test:  {X_test.shape[0]} samples ({(1-train_split)*100:.0f}%)")

        return X_train, X_test, y_train, y_test

    def create_tf_datasets(
        self,
        X_train: np.ndarray,
        y_train: np.ndarray,
        X_test: np.ndarray,
        y_test: np.ndarray,
        batch_size: int = 1024,
        shuffle_buffer: int = 10000
    ) -> Tuple[tf.data.Dataset, tf.data.Dataset]:
        """
        Create optimized TensorFlow datasets for training and testing.

        Args:
            X_train: Training features
            y_train: Training targets
            X_test: Test features
            y_test: Test targets
            batch_size: Batch size for training
            shuffle_buffer: Buffer size for shuffling training data

        Returns:
            Tuple of (train_dataset, test_dataset)
        """
        # Create training dataset
        train_ds = tf.data.Dataset.from_tensor_slices((X_train, y_train))
        train_ds = (train_ds
                    .shuffle(buffer_size=shuffle_buffer, reshuffle_each_iteration=True)
                    .batch(batch_size)
                    .prefetch(tf.data.AUTOTUNE))

        # Create test dataset (no shuffling for consistent evaluation)
        test_ds = tf.data.Dataset.from_tensor_slices((X_test, y_test))
        test_ds = (test_ds
                   .batch(batch_size)
                   .prefetch(tf.data.AUTOTUNE))

        print(f"\nTensorFlow datasets created:")
        print(f"  Batch size: {batch_size}")
        print(f"  Train batches: {len(train_ds)}")
        print(f"  Test batches: {len(test_ds)}")

        return train_ds, test_ds

    def get_datasets(
        self,
        train_split: float = 0.8,
        batch_size: int = 1024,
        shuffle_buffer: int = 10000,
        scale_data: bool = True
    ) -> Tuple[tf.data.Dataset, tf.data.Dataset, int, int, Optional[StandardScaler], Optional[StandardScaler]]:
        """

        Args:
            train_split: Fraction for training (default: 0.8)
            batch_size: Batch size (default: 1024)
            shuffle_buffer: Shuffle buffer size (default: 10000)
            scale_data: Whether to scale the data (default: True)

        Returns:
            Tuple of (train_dataset, test_dataset, input_dim, output_dim, X_scaler, y_scaler)
            Scalers are None if scale_data=False
        """
        X, y = self.load_all_files(scale_data=scale_data)

        X_train, X_test, y_train, y_test = self.create_temporal_split(
            X, y, train_split=train_split
        )

        train_ds, test_ds = self.create_tf_datasets(
            X_train, y_train, X_test, y_test,
            batch_size=batch_size,
            shuffle_buffer=shuffle_buffer
        )

        input_dim = X.shape[1]
        output_dim = y.shape[1]

        print(f"\nModel dimensions:")
        print(f"  Input:  {input_dim}")
        print(f"  Output: {output_dim}")

        # Return scalers if data was scaled .. to be used for inverse transform
        X_scaler = self.X_scaler if scale_data else None
        y_scaler = self.y_scaler if scale_data else None

        if scale_data:
            print(f"\nScalers available for inverse transform")

        return train_ds, test_ds, input_dim, output_dim, X_scaler, y_scaler


def load_multi_file_dataset(
    file_paths: List[str],
    window_length: int = 3,
    train_split: float = 0.8,
    batch_size: int = 1024,
    shuffle_buffer: int = 10000,
    scale_data: bool = True
) -> Tuple[tf.data.Dataset, tf.data.Dataset, int, int, Optional[StandardScaler], Optional[StandardScaler]]:
    """
    Quick function to load multi-file dataset with scaling.

    Example:
        files = [list of files]
        train_ds, test_ds, in_dim, out_dim, X_scaler, y_scaler = load_multi_file_dataset(files)

        # Later, to get predictions in original scale:
        y_pred_scaled = model.predict(test_ds)
        y_pred_original = y_scaler.inverse_transform(y_pred_scaled)
    """
    loader = MultiFIleDatabotLoader(file_paths, window_length=window_length)
    return loader.get_datasets(train_split, batch_size, shuffle_buffer, scale_data)
